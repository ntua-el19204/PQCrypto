...
func TestSignatureSelection(t *testing.T) {
	...
	tests := []struct {
		...
		{falcon512Cert, []SignatureScheme{Falcon512}, VersionTLS12, Falcon512, signatureFalcon512, directSigning},
		{falcon512Cert, []SignatureScheme{Falcon512}, VersionTLS13, Falcon512, signatureFalcon512, directSigning},
	...
	brokenCert := &Certificate{
		Certificate:                  [][]byte{testRSACertificate},
		PrivateKey:                   testRSAPrivateKey,
		SupportedSignatureAlgorithms: []SignatureScheme{Ed25519, Falcon512},
	}
	...
	badTests := []struct {
		...
		{falcon512Cert, []SignatureScheme{ECDSAWithP256AndSHA256, ECDSAWithSHA1}, VersionTLS12},
		{ecdsaCert, []SignatureScheme{Ed25519}, VersionTLS12},
		{ecdsaCert, []SignatureScheme{Falcon512}, VersionTLS12},
		{brokenCert, []SignatureScheme{Ed25519}, VersionTLS12},
		{brokenCert, []SignatureScheme{Falcon512}, VersionTLS12},
		...
		// it. Anyway, if a stack supports Ed25519, Falcon512 it better support sigalgs.
		{ed25519Cert, nil, VersionTLS12},
		{falcon512Cert, nil, VersionTLS12},
		...
		{falcon512Cert, nil, VersionTLS13},
...
func TestLegacyTypeAndHash(t *testing.T) {
	...
	// Falcon512 is not supported by TLS 1.0 and 1.1.
	_, _, err = legacyTypeAndHashFromPublicKey(testFalcon512PrivateKey.Public())
	if err == nil {
		t.Errorf("Falcon512: unexpected success")
	}
}
...
func TestSupportedSignatureAlgorithms(t *testing.T) {
	for _, sigAlg := range defaultSupportedSignatureAlgorithms {
		if sigAlg == Falcon512 {
			...
			if hash == 0 && sigAlg != Ed25519 && sigAlg != Falcon512 {
